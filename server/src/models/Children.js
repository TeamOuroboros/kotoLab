...
module.exports = {
	...
	async findFamilyChildren(user_id) {
		return await db(CHILDREN_TABLE).select('id', 'user_id', 'name', 'birthday', 'gender').where('user_id', user_id);
	},

	//指定した子供データを取得
	async findChild(id) {
		return await db(CHILDREN_TABLE).select('id', 'user_id', 'name', 'birthday', 'gender').where('id', id);
	},
